...
open_closed_summary <- open_closed %>%
 group_by(Date, Treatment, Time_Block)%>%
  count(Open_Closed) %>%
  mutate(Treatment = as.factor(Treatment),  Open_Closed = as.factor(Open_Closed), Open_Closed = fct_relevel(Open_Closed, "Open", "Partially open", "Closed"), Treatment = fct_relevel(Treatment, "Control", "25C", "30C"))
