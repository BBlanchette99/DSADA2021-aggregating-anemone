...
open_closed_summary <- open_closed %>%
 group_by(Date, Treatment, Time_Block)%>%
  count(Open_Closed) %>%
  mutate(Treatment = as.factor(Treatment),  Open_Closed = as.factor(Open_Closed), Open_Closed = fct_relevel(Open_Closed, "Closed", "Partially open", "Open"), Treatment = fct_relevel(Treatment, "Control", "25C", "30C"))
```
# PAM data
## Plots
```{r}
...
scale_colour_manual(values = c('#89226AFF', 
  ...
  theme(axis.text=element_text(size=15),
        ...
        legend.text=element_text(size=15), 
        legend.title=element_text(size=20))
ggsave("plots/pam_heatwave_only.png", width = 10, height = 7)
...
ggplot(pam_heatwave, aes(fill=Treatment, y=Fv_Fm_av, x=Time)) + 
  ...
  facet_grid(. ~ Day)
ggsave("plots/pam_MorningAfternoon.png")
```
...
## Ordinal regression
```{r}
open_closed_mod <- polr(Open_Closed~Time_Block+Treatment, data = open_closed_clean, Hess = TRUE)
summary(open_closed_mod)

summary_table <- coef(summary(open_closed_mod))
