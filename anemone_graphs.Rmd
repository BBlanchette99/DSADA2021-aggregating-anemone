...
#Cleaning base data
base_clean<-base%>%
  filter(Treatment != "NA", Average_Diameter != "NA", Anemone_ID != "A51B", Anemone_ID != "A42S", Anemone_ID != "A55B", Anemone_ID != "A59B", Anemone_ID != "A54B", Anemone_ID != "A25F")%>%
...
food_clean <- food%>%
  ...
  mutate(Feeding_Time_Min = as.numeric(Feeding_Time_Min),Date = as.factor(Date), Site = as.factor(Site), Treatment = fct_relevel(as.factor(Treatment), "Control", "25C", "30C"), Anemone_ID = as.factor(Anemone_ID), Bin = as.factor(Bin))%>%
