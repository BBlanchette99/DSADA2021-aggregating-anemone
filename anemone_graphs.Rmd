...
scale_colour_manual(values = c('#89226AFF', 
  ...
  theme(axis.text=element_text(size=15),
        ...
        legend.text=element_text(size=15), 
        legend.title=element_text(size=20))
ggsave(path = "plots",filename = "pam_heatwave_only.png", width = 10, height = 7)
...
ggplot(pam_heatwave, aes(fill=Treatment, y=Fv_Fm_av, x=Time)) + 
  ...
  facet_grid(. ~ Day)
ggsave(path = "plots",filename = "pam_MorningAfternoon.png")
```
...
#base_clean <- base_clean %>%
#  mutate(Bin = as.factor(Bin), Site = as.factor(Site))%>%
 # group_by(Date,Treatment, Bin)%>%
...
food_clean <- food_clean %>%
  mutate(log_Feeding_Time_Min = log(Feeding_Time_Min), Date = as.factor(Date), Site = as.factor(Site), Treatment = as.factor(Treatment), Anemone_ID = as.factor(Anemone_ID), Bin = as.factor(Bin))%>%
  select(Date, Treatment, Anemone_ID, log_Feeding_Time_Min)
